        ...
        Command::Miner { pool_url, payout_address, worker, threads } => {
            #[cfg(not(target_arch = "wasm32"))]
            {
                ...
                tokio::spawn(async move {
                    ...
                    let mut samples: VecDeque<(std::time::Instant, u64)> = VecDeque::new();
                    samples.push_back((start_time, hc.load(std::sync::atomic::Ordering::Relaxed)));
                    ...
                    loop {
                        ...
                        while samples.len() > 2 && now.duration_since(samples.front().unwrap().0).as_secs_f64() > 30.0 {
                            samples.pop_front();
                        }
                        let (old_time, old_hashes) = samples.front().copied().unwrap_or((now, current));
                        let elapsed = now.duration_since(old_time).as_secs_f64();
                        let rate = if elapsed > 0.0 {
                            current.saturating_sub(old_hashes) as f64 / elapsed
                        } else {
                            0.0
                        };
                ...
                tracing::info!("miner dashboard prints every 5s; press [ENTER] to refresh manually");
