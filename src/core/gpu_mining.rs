...
const DEFAULT_STEPS_PER_SUBMIT: u32 = 8;
...
pub struct GpuSettings {
    ...
    pub duty: f32,
    /// Number of k_step dispatches batched into one command encoder submit.
    pub steps_per_submit: u32,
    /// WGSL compute workgroup size: 64, 128, or 256.
...
impl Default for GpuSettings {
    fn default() -> Self {
        Self {
            ...
            duty: DEFAULT_DUTY,
            steps_per_submit: DEFAULT_STEPS_PER_SUBMIT,
...
impl GpuSettings {
    fn sanitized(mut self) -> Self {
        ...
        self.steps_per_submit = self.steps_per_submit.clamp(1, 64);
        self.workgroup_size = match self.workgroup_size { 128 | 256 => self.workgroup_size, _ => 64 };
        self
    }
}
...
impl GpuMiner {
    ...
    async fn from_adapter(adapter: wgpu::Adapter) -> Result<Self> {
        ...
        let shader_src = SHADER.replace("@workgroup_size(64)", &format!("@workgroup_size({})", settings().workgroup_size));
    ...
    fn groups(n: u32) -> u32 {
        let wg = settings().workgroup_size;
        (n + wg - 1) / wg
    }
    ...
    fn run_batch(
        ...
        let steps_per_submit: u32 = if throttling { 1 } else { settings().steps_per_submit };
        while remaining > 0 {
            if cancel.load(Ordering::Relaxed) {
                return None;
            }
            let k = remaining.min(chunk as u64) as u32;
            let max_steps = if k == chunk {
                ((remaining / chunk as u64) as u32).min(steps_per_submit)
            } else {
                1
            };
            ...
            for _ in 0..max_steps {
                ...
                cp.set_pipeline(&self.pipe_step);
                cp.set_bind_group(0, &self.bind_group, &[]);
                cp.dispatch_workgroups(groups, 1, 1);
                remaining -= k as u64;
            }
            ...
            let add = (n_nonces as u64).saturating_mul(k as u64 * max_steps as u64) / total;
...
pub fn mine(
    ...
    if want_gpu {
        if let Some(farm) = shared_farm() {
            return farm.mine(midstate, target, pool_target, cancel, hash_counter);
        }
...
fn adapter_rank(a: &wgpu::Adapter) -> (u8, u8) {
    let i = a.get_info();
    let type_rank = match i.device_type {
        wgpu::DeviceType::DiscreteGpu => 0u8,
        wgpu::DeviceType::IntegratedGpu => 1,
...
async fn pick_adapters(instance: &wgpu::Instance) -> Result<Vec<wgpu::Adapter>> {
    ...
    if let Some(want) = name_pref {
        ...
        adapters.retain(|a| {
            let info = a.get_info();
            info.device_type != wgpu::DeviceType::Cpu && info.name.to_lowercase().contains(&want_lc)
        });
    ...
    adapters.retain(|a| a.get_info().device_type != wgpu::DeviceType::Cpu);
    if adapters.is_empty() {
        bail!("only software (CPU) GPU adapters available");
    }
    adapters.sort_by_key(adapter_rank);
    let best_backend = adapter_rank(&adapters[0]).1;
    adapters.retain(|a| adapter_rank(a).1 == best_backend);
    Ok(adapters)
}
...
impl GpuFarm {
    ...
    async fn new_async() -> Result<Self> {
        ...
        for adapter in adapters {
            let info = adapter.get_info();
            match GpuMiner::from_adapter(adapter).await {
                Ok(g) => match g.self_test() {
                    Ok(()) => {
                        tracing::info!("GPU mining enabled on {}", g.adapter_name());
                        miners.push(Arc::new(g));
                    }
                    Err(e) => tracing::warn!("GPU mining disabled on {} (self-test failed): {e}", info.name),
                },
                Err(e) => tracing::warn!("GPU adapter {} disabled: {e}", info.name),
            }
        }
        if miners.is_empty() {
            bail!("no usable GPU passed initialization/self-test");
        }
        tracing::info!("GPU farm enabled with {} miner(s)", miners.len());
        Ok(Self { miners })
    }
    ...
    fn mine(
        ...
        if self.miners.len() == 1 {
            return self.miners[0].mine_gpu(midstate, target, pool_target, cancel, hash_counter);
        }
        let (tx, rx) = std::sync::mpsc::channel();
        for miner in &self.miners {
            let miner = Arc::clone(miner);
            let tx = tx.clone();
            let outer_cancel = Arc::clone(&cancel);
            let hash_counter = Arc::clone(&hash_counter);
            std::thread::spawn(move || {
                let local_cancel = Arc::new(AtomicBool::new(false));
                let bridge_cancel = Arc::clone(&local_cancel);
                let bridge_outer = Arc::clone(&outer_cancel);
                std::thread::spawn(move || {
                    while !bridge_outer.load(Ordering::Relaxed) {
                        std::thread::sleep(std::time::Duration::from_millis(10));
                    }
                    bridge_cancel.store(true, Ordering::Relaxed);
                });
                if let Some(hit) = miner.mine_gpu(midstate, target, pool_target, local_cancel, hash_counter) {
                    let _ = tx.send(hit);
                }
            });
        }
...
fn shared_farm() -> Option<&'static GpuFarm> {
...
where
    F: Fn(MiningResult) + Send + Sync + 'static,
{
    let want_gpu = backend() != Backend::Cpu;
    if want_gpu {
        if let Some(farm) = shared_farm() {
            let on_hit = Arc::new(on_hit);
            for miner in &farm.miners {
                let miner = Arc::clone(miner);
                ...
                std::thread::spawn(move || {
                    while !cancel.load(Ordering::Relaxed) {
                        if let Some(hit) = miner.mine_gpu(
                            midstate,
                            target,
                            pool_target,
                            Arc::clone(&cancel),
    ...
    std::thread::spawn(move || {
        while !cancel.load(Ordering::Relaxed) {
            if let Some(hit) = mine_extension(
                midstate,
                target,
                pool_target,
                threads,
